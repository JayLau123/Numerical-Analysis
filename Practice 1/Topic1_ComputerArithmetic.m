...
% exponent cannot go beyond c = 2046.（指数不能超过2046）
...
x = sqrt(2)*10^15;
y = sqrt(3);                    
sqrt_3 = (x+y)-x; %先计算x+y,比如x+y=z,再计算z-x，而非（x+y）-x=y

fprintf('x = %.16f\n',x);       % .16f says to display 16 values to the right of the decimal point.（小数点右边显示16位）
fprintf('x + y = %.16f\n\n\n',x+y);

fprintf('(x+y)-x: sqrt(3) approx = %f\n',sqrt_3);
fprintf('y=sqrt(3): actual = %f\n',sqrt(3));
...
P = @(z) sum( z.^(0:2:n) .* (-1).^(0:1:(n/2)) ./ factorial(0:2:n)); %（cos 函数的Taylor expansion）

% for given value of x, compute Taylor sum.
...
% The function sum( ) takes input of an array, and it sums the elements in the array.
% (先转换为数组array)
...
legend('Taylor polynomial','cos(x)','interpreter','latex','FontSize',15) 
% this labels the individual curves on a single plot

%% Part 4: Example nested sum
% Here we compute P(x) = 1 - 2*x + 3*x^2  + . . . （N+1）*(-1)^N*x^N evaluated at x0.

N = 7;
x0 = .2;

Px = (N+1)*(-1)^N * x0 + N*(-1)^(N-1);  
% starting value of loop
% 对每一项不断把x提到括号外，最里层就是7-8*x,也就是x的最高项8*x^7提取了x^6，只剩下8*x，前一项7*x^6只剩下7，再考虑正负号

for i = 2:N                             % nested loop
    Px = Px * x0 + (N+1-i)*(-1)^(N-i);  % 每一层乘x,再额外加一个数，比如-6+x*(7-8*x)
end
...
fp = (f(x+h)-f(x))./h;      % our numerical approximation of f'(x0). 除法/与./没区别
