...
for i = 2:steps         % run the algorithm
    k1 = v(:,:,i-1)-dt*Topic5_grad_LJ(w(:,:,i-1));
    k2 = -Topic5_grad_LJ(w(:,:,i-1) + dt*v(:,:,i-1));
    w(:,:,i) = w(:,:,i-1)+(dt/2)*(v(:,:,i-1)+k1);
    v(:,:,i) = v(:,:,i-1) + (dt/2)*(-Topic5_grad_LJ(w(:,:,i-1)) + k2);
...
xlabel('x')
ylabel('y')
title('2nd Order Modified Euler')
